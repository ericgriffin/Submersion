...
final speciesByCategoryProvider = FutureProvider.family<List<Species>, SpeciesCategory>((ref, category) async {
  final repository = ref.watch(speciesRepositoryProvider);
  return repository.getSpeciesByCategory(category);
});

/// Species search provider
final speciesSearchProvider = FutureProvider.family<List<Species>, String>((ref, query) async {
  if (query.isEmpty) {
...
final speciesProvider = FutureProvider.family<Species?, String>((ref, id) async {
  final repository = ref.watch(speciesRepositoryProvider);
  return repository.getSpeciesById(id);
});

/// Sightings for a dive provider
final diveSightingsProvider = FutureProvider.family<List<Sighting>, String>((ref, diveId) async {
  final repository = ref.watch(speciesRepositoryProvider);
...
final sightingsNotifierProvider = StateNotifierProvider.family<SightingsNotifier, List<Sighting>, String?>((ref, diveId) {
  final repository = ref.watch(speciesRepositoryProvider);
